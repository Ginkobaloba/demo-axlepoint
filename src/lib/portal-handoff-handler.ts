...
/**
 * Portal handoff handler factory (chunk 4b).
 *
 * Lives in lib/ rather than the route file because Next.js App Router route
 * modules may only export HTTP method handlers and a fixed set of config
 * keys -- exporting this factory from route.ts fails the production build's
 * route type check. Keeping it here lets the route stay legal while tests
 * still inject a stub VerifierConfig (and therefore a fake JWKS fetcher)
